...
public class InvertedIndex
{
	...
	public void addEntry(String word, int x, int y) throws IOException
	{
		...
		// ADD YOUR CODES HERE
		Vector<Posting> postings = (Vector<Posting>) hashtable.get(word);
		if (postings == null) {
			postings = new Vector<>();
		}
		...
		if (!found) {
			postings.add(new Posting("doc" + x, y));
		}
	...
	public void printAll() throws IOException
	{
		...
		// ADD YOUR CODES HERE
		FastIterator fi = hashtable.keys();
		Object key;
		while ((key = fi.next()) != null) {
			Vector<Posting> postings = (Vector<Posting>) hashtable.get(key);
			System.out.print(key + ": ");
			for (Posting p : postings) {
				System.out.print(p.doc + " " + p.freq + " ");
			}
			System.out.println();
		}
	}	
